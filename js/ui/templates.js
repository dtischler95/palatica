...
export const tpl = (function(){
  ...
  function ids(kind){
    return {
      ...
      quizCatBtn: 'vok-quizcatbtn-' + kind,
  ...
  function homePane(){
    return '<div id="vok-pane-home" class="vok-pane active">' +
      ...
      '<p class="vok-hint">' + t({ de: 'Zählt beide Sammlungen zusammen. Wiederholen senkt die Zahl, neue Einträge können sie erhöhen.', en: 'Counts both collections together. Reviewing lowers it, adding entries can raise it.' }) + '</p>' +
      '<div class="vok-hero">' +
        '<div class="vok-hero-num" id="vok-home-due">–</div>' +
        '<div class="vok-hero-lbl">' + lbl({ sr: 'доспева', de: 'fällig', en: 'due' }) + '</div>' +
      '</div>' +
  ...
  function practicePane(){
    ...
    tiles += '<div class="vok-mode-tile" data-kind="srsdeck">' +
      '<p class="vok-mode-tile-title">' + lbl({ sr: 'Шпилови', de: 'Decks', en: 'Decks' }) + '</p>' +
    ...
    var optGroups = cols.map(function(c, i){
      ...
      return '<div class="vok-opts-group' + (i === 0 ? ' active' : '') + '" data-kind="' + c.kind + '">' +
        '<div class="vok-catmenu">' +
          '<button type="button" class="vok-select vok-catmenu-btn" id="' + id.quizCatBtn + '" aria-haspopup="true" aria-expanded="false">' + line(CAT_ALL) + '</button>' +
          '<div class="vok-catmenu-panel" id="' + id.quizCat + '" hidden></div>' +
        '</div>' +
        '</div>';
    }).join('');
    optGroups += '<div class="vok-opts-group" data-kind="srsdeck">' +
      ...
      '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:2px">' +
        '<button class="vok-btn-ghost" id="vok-deck-new">' + lbl({ sr: '＋ Направи шпил', de: 'Deck erstellen', en: 'create deck' }) + '</button>' +
    ...
    return '<div id="vok-pane-practice" class="vok-pane">' +
      ...
      '<p class="vok-subh">' + lbl({ sr: 'Збирка', de: 'Sammlung', en: 'Collection' }, 'display:block') + '</p>' +
      '<p class="vok-hint">' + t({ de: 'Wörter und Sätze übst du ohne Limit. Decks folgen einem Tagespensum: nur fällige und neue Karten bis zur Tagesgrenze.', en: 'Words and sentences have no limit. Decks follow a daily quota: only due and new cards up to the daily cap.' }) + '</p>' +
      ...
      '<p class="vok-subh">' + lbl({ sr: 'Смер', de: 'Richtung', en: 'Direction' }, 'display:block') + '</p>' +
      '<p class="vok-hint">' + t({ de: 'Serbisch zuerst zeigt das serbische Wort, die Gegenrichtung fragt es aktiv ab und ist schwerer. Der Lernstand wird pro Richtung getrennt gezählt.', en: 'Serbian-first shows the Serbian word; the other direction asks you to recall it actively and is harder. Progress is tracked separately per direction.' }) + '</p>' +
      dir +
  ...
  function quizPane(){
    return '<div id="vok-pane-quiz" class="vok-pane">' +
      '<div class="vok-screen-head">' +
        ...
        '<button class="vok-btn-ghost" id="vok-quiz-undo" style="display:none" title="U">' + lbl({ sr: '↩ Врати', de: 'rückgängig', en: 'undo' }) + '</button>' +
      '</div>' +
  ...
  function deckEditPane(){
    return '<div id="vok-pane-deckedit" class="vok-pane">' +
      '<div class="vok-screen-head">' +
        ...
        '<button class="vok-icon-btn danger" id="vok-deckedit-delete" style="display:none">' + lbl({ sr: 'Обриши шпил', de: 'Deck löschen', en: 'delete deck' }) + '</button>' +
      '</div>' +
      '<p class="vok-h" style="font-size:18px;margin-bottom:4px" id="vok-deckedit-title">' + lbl({ sr: 'Направи шпил', de: 'Deck erstellen', en: 'Create deck' }) + '</p>' +
      '<p class="vok-hint">' + t({ de: 'Nur angehakte Karten gehören zum Deck, die Auswahl ändert sich nie von selbst. „Neu pro Tag" begrenzt, wie viele bisher ungeübte Karten täglich höchstens dazukommen, zusätzlich zu fälligen Wiederholungen.', en: 'Only checked cards belong to the deck, the selection never changes by itself. "New per day" caps how many never-practiced cards are added daily, on top of due reviews.' }) + '</p>' +

      '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px">' +
        '<label class="vok-field" style="flex:2;min-width:160px"><span class="vok-field-lbl">' + lbl({ sr: 'Име шпила', de: 'Deckname', en: 'Deck name' }) + '</span>' +
          '<input id="vok-deckedit-name" class="vok-input" /></label>' +
        '<label class="vok-field" style="flex:1;min-width:120px"><span class="vok-field-lbl">' + lbl({ sr: 'Нових дневно', de: 'Neu pro Tag', en: 'New per day' }) + '</span>' +
          '<input id="vok-deckedit-npd" class="vok-input" type="number" min="0" placeholder="10" /></label>' +
      '</div>' +

      '<input id="vok-deckedit-search" class="vok-input" style="margin-bottom:10px" placeholder="' + line({ sr: '🔍 претрага (реч, превод)', de: 'Suche (Wort, Übersetzung)', en: 'Search (word, translation)' }) + '" />' +
      ...
      '<div style="display:flex;gap:8px;margin-top:14px">' +
        '<button class="vok-btn" id="vok-deckedit-save" style="flex:1">' + lbl({ sr: 'Сачувај шпил', de: 'Deck speichern', en: 'save deck' }) + '</button>' +
  ...
  function listSubpane(c, i){
    ...
    return '<div id="' + id.subpane + '" class="vok-subpane' + (i === 0 ? ' active' : '') + '" data-kind="' + c.kind + '">' +
      ...
      '<p class="vok-hint">' + t({ de: 'Legt einen Eintrag sofort an, ohne durch die Übung zu gehen. Nützlich für Nachtragen und Import-Vorbereitung.', en: 'Creates an entry right away, without going through practice. Useful for catching up or preparing an import.' }) + '</p>' +
      ...
      '<div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap;align-items:center;">' +
        '<div class="vok-seg">' +
          '<button class="active" data-filter="danas">' + lbl({ sr: 'Доспева', de: 'fällig', en: 'due' }) + '</button>' +
      ...
      '<details class="vok-fold">' +
        '<summary>' + lbl({ sr: 'Управљање и подаци', de: 'Verwalten & Daten', en: 'Manage & data' }) + '</summary>' +
        ...
        '<p class="vok-hint">' + t({ de: 'Diese Aktionen löschen sofort und lassen sich nicht rückgängig machen. Ein zweiter Klick auf denselben Knopf bestätigt.', en: 'These actions delete immediately and cannot be undone. Click the same button a second time to confirm.' }) + '</p>' +
        '<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:10px">' +
          ...
          '<button class="vok-icon-btn danger" id="' + id.delToday + '">' + lbl({ sr: 'Обриши данас додато', de: 'heute Hinzugefügtes löschen', en: "delete today's additions" }) + '</button>' +
        ...
        '<p class="vok-hint">' + t({ de: 'Benennt die Kategorie bei allen Einträgen dieser Sammlung um, die sie tragen.', en: 'Renames the category on every entry in this collection that carries it.' }) + '</p>' +
        ...
        '<p class="vok-hint">' + t({ de: 'Nur diese Sammlung als JSON-Datei. Import fügt hinzu, ersetzt aber keine bestehenden Einträge.', en: 'Only this collection, as a JSON file. Import adds entries, it does not replace existing ones.' }) + '</p>' +
  ...
  function backupBlock(){
    return '<p class="vok-subh">' + lbl({ sr: 'Комплетан бекап', de: 'Voll-Backup', en: 'Full backup' }, 'display:block') + '</p>' +
      '<p class="vok-hint">' + t({ de: 'Export sichert Wörter, Sätze, Decks und den ganzen Verlauf in einer Datei. Import ersetzt restlos alles auf diesem Gerät, ohne Rückfrage außer dem Bestätigungsklick.', en: 'Export saves words, sentences, decks and the full history in one file. Import replaces everything on this device completely, with no confirmation beyond the second click.' }) + '</p>' +
      '<div style="display:flex;gap:8px;flex-wrap:wrap;">' +
        '<button class="vok-btn-ghost" id="vok-export-all">' + lbl({ sr: 'Извези све (JSON)', de: 'alles exportieren', en: 'export everything' }) + '</button>' +
        '<button class="vok-btn-ghost" id="vok-import-all-btn">' + lbl({ sr: 'Увези бекап (брише све постојеће)', de: 'Backup importieren (ersetzt alles)', en: 'import backup (replaces everything)' }) + '</button>' +
  ...
  function listPane(){
    ...
    return '<div id="vok-pane-list" class="vok-pane">' +
      '<p class="vok-h" style="font-size:18px;margin-bottom:14px">' + lbl({ sr: 'Листа', de: 'Liste', en: 'List' }) + '</p>' +
  ...
  function statsPane(){
    return '<div id="vok-pane-stats" class="vok-pane">' +
      ...
      '<p class="vok-hint">' + t({ de: 'Jede bewertete Karte zählt, egal ob nochmal, halb oder gewusst.', en: 'Every graded card counts, whether rated again, half or known.' }) + '</p>' +
      ...
      '<p class="vok-hint">' + t({ de: 'Ein Eintrag zählt hier erst, sobald beide Richtungen die Lernschwelle erreicht haben.', en: 'An entry only counts here once both directions have reached the learned threshold.' }) + '</p>' +
  ...
  function settingsPane(){
    return '<div id="vok-pane-settings" class="vok-pane">' +
      ...
      '<p class="vok-hint">' + t({ de: 'Nur die Anzeige wechselt zwischen Kyrillisch und Latein, gespeichert bleibt der serbische Text immer kyrillisch.', en: 'Only the display switches between Cyrillic and Latin, the stored Serbian text always stays Cyrillic.' }) + '</p>' +
      '<div class="vok-seg wide">' +
      ...
      '<p class="vok-hint">' + t({ de: 'Die zweite Sprache neben Serbisch: Übersetzungen, Menüs und Hinweise folgen dieser Wahl.', en: 'The second language alongside Serbian: translations, menus and hints follow this choice.' }) + '</p>' +
      '<div class="vok-seg wide">' +
      ...
      '<p class="vok-subh">' + lbl({ sr: 'План понављања', de: 'Wiederholungsplan', en: 'Review plan' }, 'display:block') + '</p>' +
      '<p class="vok-hint">' + t({ de: 'Abstände in Tagen bis zur nächsten Wiederholung, aufsteigend, ab 1. Jede Stufe gilt nach „weiß ich", davor beginnt die Reihe wieder bei der ersten Zahl.', en: 'Gaps in days until the next review, ascending, starting at 1. Each step applies after "know it", a miss resets to the first number.' }) + '</p>' +
      ...
      '<p class="vok-hint">' + t({ de: 'Erreicht der Wiederholungsabstand diese Zahl an Tagen, gilt der Eintrag in dieser Richtung als gelernt.', en: 'Once the review interval reaches this many days, the entry counts as learned in that direction.' }) + '</p>' +
      '<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:6px">' +
      ...
      '<p class="vok-subh">' + lbl({ sr: 'Синхронизација између уређаја', de: 'Sync zwischen Geräten', en: 'Sync across devices' }, 'display:block') + '</p>' +
      '<p class="vok-hint">' + t({ de: 'Optional. Gleicht den Lernstand über ein eigenes, kostenloses Supabase-Projekt ab. Ohne Einrichtung bleibt alles rein lokal auf diesem Gerät.', en: 'Optional. Syncs progress via your own free Supabase project. Without setup, everything stays purely local on this device.' }) + '</p>' +
      '<div id="vok-cloud-panel" class="vok-card" style="flex-direction:column;align-items:stretch;gap:8px">' +
        ...
        '<div id="vok-cloud-setup" style="display:none">' +
          '<p class="vok-status" style="margin:0 0 6px">' + t({ de: 'Ein eigenes Supabase-Projekt anlegen und dessen Zugang hier eintragen. Die Daten liegen dann in deinem Projekt, nicht bei jemand anderem.', en: 'Create your own Supabase project and enter its access here. Your data then lives in your project, not with anyone else.' }) + '</p>' +
        ...
        '<div id="vok-cloud-account" style="display:none">' +
          ...
          '<button class="vok-btn-ghost" id="vok-cloud-push" style="width:100%;margin-bottom:6px">' + lbl({ sr: 'Пренеси локалне податке у облак (не брише ништа)', de: 'lokale Daten in die Cloud übernehmen (überschreibt nichts)', en: 'move local data to the cloud (overwrites nothing)' }) + '</button>' +
          '<div style="display:flex;gap:8px;flex-wrap:wrap">' +
            '<button class="vok-btn-ghost" id="vok-cloud-signout" style="flex:1">' + lbl({ sr: 'Одјава', de: 'abmelden', en: 'sign out' }) + '</button>' +
            '<button class="vok-icon-btn danger" id="vok-cloud-forget" style="flex:1">' + lbl({ sr: 'Уклони приступ са овог уређаја', de: 'Zugang von diesem Gerät entfernen', en: 'remove access from this device' }) + '</button>' +
          '</div>' +
        '</div>' +
